...
def main():
    ...
    model_version = 30 # which version of the model are we investigating? 30, 100 or 200 topics?
    ...
    topic_n = 12 # which topic are we investigating?

    documents_for_topic = get_documents_for_topic(topic_n, model, dictionary, corpus)


def get_document(n_of_document):
    with open('/Volumes/Transcend/Documents/GRADU/Tiedostot/uudet_lemmat/file%s.txt.lemma' % (n_of_document)) as file:
        text = file.readline()
...
def get_topics_for_document(document, dictionary, model):
    ...
    topics = model.get_document_topics(bow, minimum_probability = 0.4)
    return topics
